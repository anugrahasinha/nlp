...
class Zomato:
    ...
    def restaurant_search_sort(self, query="", latitude="", longitude="", cuisines="",start=0,limit=5,sort_by='cost',order="asc"):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.
        """
        ...
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        if sort_by!='':
            url=base_url + "search?q=" + str(query) +"&start=" +str(start)+"&count="+str(limit)+ "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines)+"&sort="+sort_by+"&order="+order
        else:
            url=base_url + "search?q=" + str(query) +"&start=" +str(start)+"&count="+str(limit)+ "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines)
        #print(url)
